...
import type { SettingsResponse, TtsVoice } from "../lib/settingsApi";
...
function Settings() {
	...
	const [selectedVoice, setSelectedVoice] =
		useState<TtsVoice>("en-US-GuyNeural");
	...
	const [sttModels, setSttModels] = useState<
		import("../lib/settingsApi").SttModelInfo[]
	>([]);
	...
	const handleModelChange = async (
		...
		const isInstalled =
			settingsData?.installedModels.includes(newModel) ?? false;
		...
		if (!isInstalled) {
			...
			try {
				...
				while (true) {
					...
					if (value.status) {
						setDownloadStatus(
							value.status === "success" ? "Download complete!" : value.status,
						);
					}
					if (value.message) {
	...
	const handleVoiceChange = async (
		...
		try {
			...
			// Revert to previous value
			setSelectedVoice(
				(settingsData?.ttsVoice as TtsVoice) || "en-US-GuyNeural",
			);
		} finally {
	...
	/**
	 * Handle TTS rate slider change - only update local state during drag
	 */
	...
	/**
	 * Handle TTS rate slider commit - save when user releases slider
	 */
	const handleRateChangeCommit = async (
		event:
			| React.ChangeEvent<HTMLInputElement>
			| React.MouseEvent<HTMLInputElement>,
	) => {
		const target = event.target as HTMLInputElement;
		const newRate = parseInt(target.value, 10);
		setRateValue(newRate);
	...
	/**
	 * Handle TTS pitch slider change - only update local state during drag
	 */
	...
	/**
	 * Handle TTS pitch slider commit - save when user releases slider
	 */
	const handlePitchChangeCommit = async (
		event:
			| React.ChangeEvent<HTMLInputElement>
			| React.MouseEvent<HTMLInputElement>,
	) => {
		const target = event.target as HTMLInputElement;
		const newPitch = parseInt(target.value, 10);
		setPitchValue(newPitch);
	...
	const handleSttModelChange = async (
		event: React.ChangeEvent<HTMLSelectElement>,
	) => {
		const newModel = event.target.value;
	...
	const ttsSettingsProps: TTSSettingsProps = {
		...
		onRateChangeCommit: handleRateChangeCommit,
		onPitchChange: handlePitchChange,
		onPitchChangeCommit: handlePitchChangeCommit,
	};
	...
	return (
		<div
			...
			aria-label="Settings"
		>
			<h2 className="settings-page__title" id="settings-title">
				Settings
			</h2>
			...
			<section className="settings-section" aria-labelledby="model-heading">
				<h3 className="settings-section__title" id="model-heading">
					AI Model Selection
				</h3>
			...
			<section className="settings-section" aria-labelledby="tts-heading">
				<h3 className="settings-section__title" id="tts-heading">
					Voice Output
				</h3>
			...
			<section className="settings-section" aria-labelledby="stt-heading">
				<h3 className="settings-section__title" id="stt-heading">
